function [I,t_consume,obj]=upSamp_ell1_noisy(DataFilter,grid,I0,ell,mask,max_iter,epsilon,I_ini)
...
% variables. The data-fidelity constraint is no longer an exact equality
% but rather: \|Phi*I-I0\|^2<=epsilon^2
% 
t0=tic;
...
w=I;
r=ones(s_I);
p=reshape(downSamp(imfilter(w,DataFilter,'circular','conv'),grid),s_I0)-I0;

lambda=ell/max(DataFilter(:));20;%;
rho=.5*max(mask(:))*ell;%
gamma=rho;
% mask=mask(:);
...
for count=1:max_iter
    ...
    I_ft=(rho.*(fft2(u1+v1).*Dr_conj_ft+fft2(u2+v2).*Di_conj_ft)...
        +gamma.*fft2(w+r))./(LtL_ft+rho.*DtD_ft+gamma);
    ...
    % w update
    PhiT_I0pz_Ir=lambda.*imfilter(upSamp(I0+p-z,grid,s_I),DataFilter_dual,...
        'circular','conv')+gamma.*(I-r);
    w=1/gamma.*(PhiT_I0pz_Ir-lambda.*imfilter(upSamp(ifft2(fft2(reshape(downSamp(...
        imfilter(PhiT_I0pz_Ir,DataFilter,'circular','conv'),...
    ...
    Phi_w_z_I0=Phi_w+z-I0;
    norm_Phi_w_z_I0=norm(Phi_w_z_I0(:),2);
    if norm_Phi_w_z_I0>epsilon
        p=(epsilon/norm_Phi_w_z_I0).*Phi_w_z_I0;
    else
        p=Phi_w_z_I0;
    end
